 ...
 return (x - in_min) * (out_max - out_min) / (in_max - in_min) + out_min;
  ...
  void callback(const sensor_msgs::Joy::ConstPtr& msg)
  {
    beginner_tutorials::motion m;
     ...
     m.RL = mapfloat(msg->axes[6] , -1, 1, 0, 1000);
     m.FB = mapfloat(msg->axes[7] , -1, 1, 0, 1000);
     m.RL2 = mapfloat(msg->axes[2] , -1, 1, 0, 1000);
     m.FB2 = mapfloat(msg->axes[3] , -1, 1, 0, 1000);
     m.RT = mapfloat(msg->axes[4] , 1, -1, 0, 255);
     m.LT = mapfloat(msg->axes[5] , 1, -1, 0, 255);

     ROS_INFO("ForwardBackward: [%i]", m.FB);
     ROS_INFO("Rightleft: [%i]", m.RL);
     ROS_INFO("UPDOWNMECH: [%i]", m.FB2);
     m.A = msg->buttons[0];
     m.B = msg->buttons[1];
     m.Y = msg->buttons[2];
     m.X = msg->buttons[3];

    ROS_INFO("Buttons A B Y X W E S: %i %i %i %i", m.A ,m.B,m.Y,m.X);
    pub_.publish(m);
