...
double DistancePerCount = (3.14159265 * 0.0263)/2000; //the wheel diameter is 0.1m last calibrated = 0.0287 574\ lma bykbr el sor3a tzeed
...
double roll, pitch, yaw, referenceyaw, diffrenceyaw;
...
ROS_INFO("YAW : %f",yaw);
diffrenceyaw = yaw - referenceyaw;
if(diffrenceyaw > PI) diffrenceyaw -= TwoPI;
else if (diffrenceyaw < -PI) diffrenceyaw += TwoPI;

ROS_INFO("reference : %f",referenceyaw);
ROS_INFO("Diff : %f",diffrenceyaw);

if(abs(diffrenceyaw) >= 1.3 && abs(diffrenceyaw) <= 1.6 && !justturned) {
if(diffrenceyaw > 0) { 

th = th + (PI/2); referenceyaw += (PI/2); 
Direction.data ++;
if(Direction.data > 4) Direction.data = Direction.data - 4;
}
else if(diffrenceyaw < 0) { 
th =th - (PI/2);  referenceyaw -= (PI/2); 
Direction.data --;
if(Direction.data < 1) Direction.data = Direction.data + 4;
}
if(referenceyaw > TwoPI) referenceyaw -= TwoPI;
else if(referenceyaw < -TwoPI) referenceyaw += TwoPI;
justturned=true;
ROS_INFO("reference : %f",referenceyaw);
ROS_INFO("Diff : %f",diffrenceyaw);
}
if(justturned) { justturned=false;}
...
int main(int argc, char **argv)
{
 ...
 ros::Publisher dir_pub = n.advertise<std_msgs::Int64>("directions", 50); 
Direction.data = 1;
