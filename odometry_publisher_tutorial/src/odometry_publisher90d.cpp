...
if(firstyaw) { referenceyaw = yaw + thshift; firstyaw = false; }
//ROS_INFO("YAW : %f",yaw);
if(abs(yaw - referenceyaw) > 1.45 && !justturned) {
if(yaw - referenceyaw > 0) th = th + (PI/2);
else if(yaw - referenceyaw < 0) th =th - (PI/2);
referenceyaw = yaw - referenceyaw;
justturned=true;
//ROS_INFO("%f",referenceyaw);
} else if(justturned && abs(yaw - referenceyaw) < 1.45) { justturned=false;}
